...
- A brown dwarf can only orbit two stars if both are from the same system. (TRIGGER necessary)
...
- Uma anã marrom só pode orbitar duas estrelas se ambas estiverem no mesmo sistema. (TRIGGER necessário)
